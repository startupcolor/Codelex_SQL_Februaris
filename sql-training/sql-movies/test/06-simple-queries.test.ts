...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {    
      ...
      const query = `SELECT round(SUM(budget_adjusted),2 ) as total_budget,        
       round(SUM(revenue_adjusted),2 ) as total_revenue FROM ${MOVIES}`; 
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT (release_date) as count FROM ${MOVIES}      
  ...
  it(
    ...
    async done => {
      const query = `SELECT original_title, budget, revenue FROM ${MOVIES}
      WHERE (budget > 100000000 AND release_date > '2009-01-01')
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT(*) as "count",strftime('%Y',RELEASE_DATE) as "year" FROM MOVIES
      group by strftime('%Y',RELEASE_DATE) order by strftime('%Y',RELEASE_DATE) desc`;
  ...
  it("should select top three users which left most ratings",
    async done => {
    
      const query = `SELECT user_id, count(*) as "count" From movie_ratings
  ...
  it(
    ...
    async done => {
      const query = `SELECT strftime('%m',time_created) as "month" ,count(*) as "count" From movie_ratings
      Group by strftime('%m',time_created) Order by count desc`;
      const result = await db.selectMultipleRows(query);
